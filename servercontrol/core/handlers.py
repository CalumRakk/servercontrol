...
from .commands import echo, end_aws, get_aws_status, help


def register_handlers(app: Client, config: LabConfig): 

    app.add_handler(MessageHandler(echo, filters.text & filters.private))
    app.add_handler(MessageHandler(lambda client, message: get_aws_status(client, message, config), filters.command("status")))
